...
STnode* NEW(Item item, STnode* l, STnode* r, int N) {
    ...
    x->item.am = item.am;
    strcpy(x->item.firstname, item.firstname);
    strcpy(x->item.lastname, item.lastname);
...
int count_how_many_times_a_number_appears(STnode* root, Key num) {
    if (root == NULL) {
        return 0;
    }

    Key root_key = key(root->item);

    if (eq_key(root_key, num)) {
        return 1 + count_how_many_times_a_number_appears(root->l, num) + count_how_many_times_a_number_appears(root->r, num);
    } else if (less_key(root_key, num)) {
