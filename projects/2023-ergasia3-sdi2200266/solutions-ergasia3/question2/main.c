...
Graph* loadFile(const char* filename) {
    ...
    while (fgets(buffer, 4000, fp)) {
        ...
        if (isalpha(first[0])) {
            ...
            printf("Creating edge %d (%c) to %d (%c) for file %s \n", n1, first[0], n2, second[0], filename);
            InsertEdge(g, n1, n2);
        } else {
            Vertex n1 = atoi(first);
            Vertex n2 = atoi(second);

            printf("Creating edge %d to %d for file %s \n", n1, n2, filename);
            InsertEdge(g, n1, n2);
        }
    }
...
int main(int argc, char* argv[]) {
    Graph* g1 = loadFile("graph_edge_p26.txt");
    Graph* g2 = loadFile("graph_edge_p69.txt");
    Graph* g3 = loadFile("graph_edge_p164.txt");

    if (g1 == NULL) {
        printf("Error while reading .. graph_edge_p26 \n");
        return 1;
    }

    if (g2 == NULL) {
        printf("Error while reading .. graph_edge_p69 \n");
        return 1;
    }

    if (g3 == NULL) {
        printf("Error while reading .. graph_edge_p164 \n");
        return 1;
    }
